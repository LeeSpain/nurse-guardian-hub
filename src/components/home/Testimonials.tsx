      ...
      <div className="container mx-auto px-4 relative">
        <Transition animation="fade-up">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-purple-700 via-purple-600 to-purple-800">What Our Users Say</h2>
...
const TestimonialCard: React.FC<TestimonialCardProps> = ({ testimonial }) => {
  ...
  return (
    <div className="relative h-full">
      ...
      {/* Card content */}
      <div className="relative bg-white rounded-2xl p-8 shadow-xl border border-gray-100 h-full z-10 backdrop-blur-sm">
        {/* Quote icon */}
        <div className={`absolute -top-4 -left-4 w-10 h-10 rounded-full flex items-center justify-center text-white bg-gradient-to-br ${gradientBg}`}>
          <Quote size={20} />
        </div>
        ...
        <div className="mb-4 flex">
          ...
          {[...Array(5 - testimonial.stars)].map((_, i) => (
            <Star key={i + testimonial.stars} className="h-5 w-5 text-gray-200" />
          ))}
        </div>
        
        {/* Quote text */}
        <p className="text-gray-700 mb-6 font-medium italic">{testimonial.quote}</p>
        ...
        <div className="flex items-center">
          <div className="mr-4 relative">
            ...
            <img 
              ...
              className={`h-14 w-14 rounded-full border-2 ${testimonial.type === 'nurse' ? 'border-purple-300' : 'border-client'} object-cover`}
            />
          </div>
          <div>
            <p className="font-semibold text-gray-800">{testimonial.author}</p>
            <p className={`text-sm ${testimonial.type === 'nurse' ? 'text-purple-600' : 'text-client'}`}>
              {testimonial.role}
            </p>
          </div>
        </div>
